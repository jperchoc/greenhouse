import GreenhouseWindow from "./actuators/greenhouse_window.js";
import LEDs from "./actuators/led.js";
import Pump from "./actuators/pump.js";
import { influxWrite, initInflux } from "./helpers/influx.js";
...
import GreenhouseAPI from "./server.js";
...
const windowActuators = config.actuators.windows.map(actuatorConf => new GreenhouseWindow(actuatorConf));
...
const getSunset = async () => {
    try {
        ...
        const response = await fetch(`http://api.sunrise-sunset.org/json?lat=${lat}&lng=${long}&date=today&formatted=0`);
